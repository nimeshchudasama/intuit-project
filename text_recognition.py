...
for ((startX, startY, endX, endY), text) in results:
    ...
    # Write the outputs into output folder
    try:
        os.makedirs("output")
    except FileExistsError:
        pass
    cv2.imwrite("output/output" + str(count) + ".jpg", output)
    count += 1
    
    # cv2.imwrite() doesn't work in Google Colab. Used cv2_imshow() from google.colab.patches
    cv2_imshow(output)
    cv2.waitKey(0)